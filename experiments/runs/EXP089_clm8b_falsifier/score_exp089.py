"""EXP089 scoring — zero-shot cosine state-vs-option scoring + §7.1 verdict logic.
...
Pipeline:
  1. Load embeddings (state, actA, actC) from exp089_embeddings.npz.
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="EXP089 zero-shot scoring")
    ap.add_argument("--embeddings", required=True,
                    help="path to exp089_embeddings.npz")
    ...
    try:
        ...
        return 3
    # F1 (Law #14 LOG-345): stamp execution mode into the report. The mode
    # comes from the extraction meta colocated with the embeddings bundle
    # (written by extract_mock/extract_real); absent meta → "unknown".
    meta_path = os.path.join(os.path.dirname(os.path.abspath(args.embeddings)),
                             "exp089_extraction_meta.json")
    mode = "unknown"
