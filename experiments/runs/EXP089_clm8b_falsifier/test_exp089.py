"""EXP089 unit tests — ALL must pass. Synthetic/mock only; no weights touched.

Covers: verdict-boundary cases (38/60, 37/60, 39/60), split-tie PIVOT,
opposite-tilt PIVOT, G4 deaf-instrument RUN-INVALID, tie-breaking, G1/G2/G3
guard logic, binomial exactness, runner refusal rules, and the signed-protocol
digest guard. numpy-only (no torch required).
"""
...
class TestBenchmark(unittest.TestCase):
    ...
    def test_archive_mismatch_fires_guard(self):
        # KNOWN PROVENANCE BREAK (LOG-3994, LOG-4238): the repo's repaired
        # benchmark builder does NOT reproduce the smoke archive's (ent,typ)
        # sequence — the archive (Mars×6, Venus×6, …) was produced by the
        # pre-repair builder, whose code is not retained in-repo (original
        # zip 286c9576… superseded, not on disk; on-disk zip 936dbce4…
        # embeds the repaired runner ea740625…). The protocol-mandated guard
        # MUST fire here. This test pins that it fires rather than silently
        # passing — 12/60 (ent,typ) matches, first mismatch at index 1
        # (rebuilt Venus/planet vs archived Mars/planet), exactly as LOG-4238.
...
class TestRunnerRefusals(unittest.TestCase):
    def test_refuses_without_clearance(self):
        with tempfile.TemporaryDirectory() as d:
            self.assertEqual(run_exp089.main(["--out-dir", d]), 2)

    def test_refuses_forbidden_flags(self):
        with tempfile.TemporaryDirectory() as d:
            for flag in ("--gpu", "--train", "--cuda", "--finetune"):
                self.assertEqual(
                    run_exp089.main(["--out-dir", d, "--mock", flag]), 2,
                    f"flag {flag} was not refused")

    def test_signed_protocol_digest_guard_passes(self):
        self.assertTrue(run_exp089.assert_signed_protocol())

    def test_mock_end_to_end(self):
        with tempfile.TemporaryDirectory() as d:
            rc = run_exp089.main(["--out-dir", d, "--mock"])
            self.assertEqual(rc, 0)
            rp = os.path.join(d, "exp089_report.json")
            ...
            self.assertTrue(os.path.isfile(
                os.path.join(d, "exp089_embeddings.npz")))
...
class TestModeStamp(unittest.TestCase):
    """F1 (Law #14 LOG-345): the verdict artifact must carry its mode."""
    ...
    def _bundle(self, d, mode=None):
        ...
        phrasing = np.array(["A-first"] * 30 + ["C-first"] * 30)
        ids = np.array([f"item_{i}" for i in range(n)])
        p = os.path.join(d, "exp089_embeddings.npz")
        np.savez(p, state=s, actA=a, actC=c, ids=ids, phrasing=phrasing)
        if mode is not None:
            with open(os.path.join(d, "exp089_extraction_meta.json"), "w") as f:
    ...
    def test_scorer_main_stamps_mock_mode(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(rc, 0)
            with open(os.path.join(out, "exp089_report.json")) as f:
                rep = json.load(f)
    ...
    def test_runner_mock_stamps_mock_mode(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(rc, 0)
            with open(os.path.join(d, "exp089_report.json")) as f:
                rep = json.load(f)
            self.assertEqual(rep["mode"], "mock")
