"""EXP091 scoring — zero-shot cosine state-vs-option scoring + §7.1 verdict logic.
...
Scoring logic is BYTE-IDENTICAL to the EXP090 scorer (only the module/report
names changed): the registered question, bar, and decision tree did not
change across the EXP090 → EXP091 succession.

Pipeline:
  1. Load embeddings (state, actA, actC) from exp091_embeddings.npz.
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="EXP091 zero-shot scoring")
    ap.add_argument("--embeddings", required=True,
                    help="path to exp091_embeddings.npz")
    ...
    out_path = os.path.join(args.out_dir, "exp091_report.json")
