"""EXP091 runner — CLI entry point.

Launch-chain position (signed protocol §12): signed pre-registration (LOG-357)
...
Modes:
  ...
  --ceo-clearance  REAL execution: frozen snapshot extraction (CPU, read-only)
                   + scoring. Requires torch + transformers + accelerate
                   installed.
...
from benchmark_exp091 import SIGNED_PROTOCOL_DIGEST, signed_protocol_path
...
def main(argv=None):
    ...
    for flag in FORBIDDEN_FLAGS:
        if any(a == flag or a.startswith(flag + "=") for a in argv):
            print(f"REFUSAL: forbidden flag {flag} — this protocol licenses "
                  ...
                  file=sys.stderr)
            return 2
    ap = argparse.ArgumentParser(description="EXP091 runner")
    ...
    from score_exp091 import decide_from_embeddings
    ...
    try:
        ...
        # authoritative verdict artifact — a mock-run KILL must be
        # distinguishable on disk from a real-run KILL.
