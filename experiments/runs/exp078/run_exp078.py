"""
EXP078: Subspace-Restricted Bridge -- Is the Subspace Right and the Direction Wrong?
Pre-registered confirmatory protocol:
    experiments/protocols/EXP078_SUBSPACE_BRIDGE_REREG_PREREG_SPEC.md
...
Output directory: experiments/runs/EXP078_subspace_bridge/
...
Governing standards:
    ...
    - reports/adversarial_review_exp078_prereg_2026-09-23.md (SIGNED)
...
def main():
    parser = argparse.ArgumentParser(description="EXP078: subspace-restricted bridge (pre-registered)")
    ...
    out_dir = os.path.join("experiments", "runs", "EXP078_subspace_bridge")
    os.makedirs(out_dir, exist_ok=True)
    log_path = os.path.join(out_dir, "exp078_run_log.txt")
    ...
    log("=" * 80, log_file)
    log("EXP078: Subspace-Restricted Bridge (boundary science, not novelty science)", log_file)
    log("Pre-registered protocol: experiments/protocols/EXP078_SUBSPACE_BRIDGE_REREG_PREREG_SPEC.md", log_file)
    ...
    if not torch.cuda.is_available():
        if not args.allow_cpu:
            msg = ("FATAL: no CUDA GPU detected. EXP078 requires a GPU "
    ...
    def save_halt(outcome, reason, extra):
        ...
        with open(os.path.join(out_dir, "exp078_results.json"), "w", encoding="utf-8") as f:
            json.dump(payload, f, indent=2)
        log("=" * 80, log_file)
        log(f"{outcome}: {reason}", log_file)
        log("THIS HALT IS THE REPORTABLE OUTCOME OF EXP078 (protocol §7.2).", log_file)
        log(f"Diagnostics written to {out_dir}/exp078_results.json", log_file)
        log("=" * 80, log_file)
        log_file.close()
    ...
    if not (rank_ratio > RANK_GUARD_RATIO):
        save_halt("HALT_RANK",
                  ...
                  f"re-run under EXP078.",
                  {"rank_ratio": rank_ratio, "singular_values": svals})
        print("RANK GUARD HALTED — THIS IS THE RESULT. Download exp078_results.json and report it.")
        sys.exit(0)
    ...
    for i, (iA, iB, iC) in enumerate(TRIPLES_INDICES):
        ...
        _add(f"exp078_planet_2hop_{i}", 2, "Planetary", p, A, C)
    for i, (iA, iB, iC, iD) in enumerate(QUADS_INDICES):
        ...
        _add(f"exp078_planet_3hop_{i}", 3, "Planetary", p, A, D_ent)
    for i, (iA, iB, iC) in enumerate(TRIPLES_INDICES):
        ...
        if i < 7:
            ...
            p = f"Premise: {C} is lower than {B}. {B} is lower than {A}. Question: Who is higher in rank, {q_opts}? Answer:"
        _add(f"exp078_element_2hop_{i}", 2, "Elemental", p, A, C)
    for i, (iA, iB, iC, iD) in enumerate(QUADS_INDICES):
        ...
        if i < 7:
            ...
            p = f"Premise: {D_ent} is lower than {C}. {C} is lower than {B}. {B} is lower than {A}. Question: Who is higher in rank, {q_opts}? Answer:"
        _add(f"exp078_element_3hop_{i}", 3, "Elemental", p, A, D_ent)
    ...
    def make_bridge_vec(t_tok_idx, f_tok_idx):
        # LOG-114 mechanical fix: transformers 5.x renamed GPTNeoXForCausalLM.embed_out
        # to lm_head; get_output_embeddings() is version-agnostic (4.44+ and 5.x) and
        # returns the same unembedding matrix. Zero scientific change.
        out_emb = model.get_output_embeddings().weight
        w = out_emb[t_tok_idx, :].detach() - out_emb[f_tok_idx, :].detach()
        return w / (torch.norm(w) + 1e-12)

    proj_S = Q_S @ Q_S.T                      # [d, d]
    # LOG-116 mechanical fix: Q_S is CPU-built (support deltas are .detach().cpu()
    # by construction, L371/382) while bridge vectors inherit the model's device
    # (cuda when available, via model.to(device) L274). Move proj_S to the model
    # device so the projection matmuls below are device-consistent. Values
    # identical; placement only. Zero scientific change. (Caught on the GPU run;
    # the CPU smoke never saw it because everything is CPU there.)
    proj_S = proj_S.to(device)
    ...
    if not (e_median >= ENERGY_BAR):
        save_halt("HALT_ENERGY",
                  ...
                  f"Re-design required, not a re-run under EXP078 (reportable per §7.2).",
                  {"e_median": e_median, "e_distribution": {
                   ...
                   "rank_ratio": rank_ratio, "singular_values": svals})
        print("ENERGY GATE HALTED — THIS IS THE RESULT. Download exp078_results.json and report it.")
        sys.exit(0)
    ...
    if not (HEADROOM_LO <= base_acc <= HEADROOM_HI):
        ...
        print("HEADROOM GATE HALTED — THIS IS THE RESULT. Download exp078_results.json and report it.")
        sys.exit(0)
    ...
    torch.save(archive, os.path.join(out_dir, "exp078_vectors.pt"))
    log("Vector archive written: exp078_vectors.pt (Law #13).", log_file)
    ...
    res_json_path = os.path.join(out_dir, "exp078_results.json")
    with open(res_json_path, "w", encoding="utf-8") as f:
        json.dump(results_payload, f, indent=2)
    inst_json_path = os.path.join(out_dir, "exp078_instance_records.json")
    with open(inst_json_path, "w", encoding="utf-8") as f:
        json.dump(instance_records, f, indent=2)

    log(f"\nArtifacts saved to:\n  {res_json_path}\n  {inst_json_path}\n"
        f"  {os.path.join(out_dir, 'exp078_vectors.pt')} (Law #13 archive)\n  {log_path}", log_file)
    log("Run evaluate_exp078.py on exp078_results.json for the pre-registered decision-tree ruling.", log_file)
    log_file.close()
    print("EXP078 COMPLETED. Now run: python experiments/runs/exp078/evaluate_exp078.py")
