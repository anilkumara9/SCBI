"""
EXP078 decision-tree evaluator (protocol §7, pre-registered, SIGNED).

Reads exp078_results.json and outputs the pre-registered ruling branch VERBATIM.
...
Conservative readings (documented, mirroring run_exp078.py):
...
Usage:
    python evaluate_exp078.py [path/to/exp078_results.json]
"""
...
RULINGS = {
    "a": ("Branch (a): energy gate failed (median e < 0.10). RULING: HALT -- "
          ...
          "Re-design required (not a re-run under EXP078)."),
...
def report(norm, branch_letter):
    ...
    lines = []
    lines.append("=" * 80)
    lines.append("EXP078 DECISION-TREE RULING (protocol §7, pre-registered)")
    lines.append("=" * 80)
...
def main():
    ap = argparse.ArgumentParser(description="EXP078 pre-registered decision-tree evaluator")
    ap.add_argument("results", nargs="?", default=None,
                    help="path to exp078_results.json (default: experiments/runs/EXP078_subspace_bridge/exp078_results.json)")
    args = ap.parse_args()
    path = args.results or os.path.join("experiments", "runs",
                                         "EXP078_subspace_bridge",
                                         "exp078_results.json")
    with open(path, encoding="utf-8") as f:
        payload = json.load(f)
    norm = normalize_exp078(payload)
