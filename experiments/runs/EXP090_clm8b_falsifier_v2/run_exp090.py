"""EXP090 runner — CLI entry point.

Launch-chain position (signed protocol §12): signed pre-registration (LOG-350)
...
from benchmark_exp090 import SIGNED_PROTOCOL_DIGEST, signed_protocol_path
...
def main(argv=None):
    ...
    for flag in FORBIDDEN_FLAGS:
        if any(a == flag or a.startswith(flag + "=") for a in argv):
            print(f"REFUSAL: forbidden flag {flag} — this protocol licenses "
                  ...
                  file=sys.stderr)
            return 2
    ap = argparse.ArgumentParser(description="EXP090 runner")
    ...
    from score_exp090 import decide_from_embeddings
