# EXP075 RUNBOOK — free-tier execution (Kaggle first, Colab backup)

**Cost: $0.** No paid GPU, no API keys, no sign-ups beyond a free Kaggle/Google account.

**What you are running:** the pre-registered EXP075 protocol
(`experiments/protocols/EXP075_SUBSPACE_BRIDGE_PREREG_SPEC.md`) — a
...
3b. **Pre-flight GPU check (new cell, run before anything else).** If this
   ...
   ```
4. Upload the bundle. The execution bundle is the whole `experiments/runs/exp075/`
   directory (`run_exp075.py`, `evaluate_exp075.py`, `test_evaluate_exp075.py`,
   ...
   - Kaggle → Create → New Notebook → File → Upload: upload all files in
     `experiments/runs/exp075/` (or zip them and unzip in the first cell).
5. In the first cell, install dependencies and verify:
   ```python
   !pip install -q -r requirements.txt
   !python3 -m py_compile run_exp075.py evaluate_exp075.py test_evaluate_exp075.py
   !python3 test_evaluate_exp075.py     # expect: ALL EXP075 EVALUATOR BRANCH TESTS PASSED
...
6. Run (from the directory containing the bundle files):
   ```python
   !python3 run_exp075.py
   ```
   - Outputs land in `experiments/runs/EXP075_subspace_bridge/` (created
...
7. Get the ruling (in the same notebook, or locally after download):
   ```python
   !python3 evaluate_exp075.py
   ```
   This prints one of the pre-registered branches (a/a'/b/c/d/e/f/g/h/i)
   VERBATIM. That printed ruling is the result of EXP075.
8. **Download:** in a final cell:
   ```python
   !tar czf /kaggle/working/exp075_artifacts.tar.gz -C experiments/runs EXP075_subspace_bridge
   ```
   then download `/kaggle/working/exp075_artifacts.tar.gz` from the output panel.
...
5. Download via `files.download()`:
   ...
   !tar czf /content/exp075_artifacts.tar.gz -C /content/experiments/runs EXP075_subspace_bridge
   files.download('/content/exp075_artifacts.tar.gz')
   ```

## What success looks like (checklist)

- [ ] `exp075_run_log.txt` shows: model loaded → pre-hash logged →
...
- [ ] `exp075_results.json` has `"outcome": "COMPLETED"` and
      `"protocol_scope": "IN-SCOPE (pythia-410m, layer 20)"`.
- [ ] `evaluate_exp075.py` prints one of the pre-registered branches.
- [ ] `exp075_vectors.pt` exists (Law #13 archive: Q_S, v_hat_k, per-item
      projections pre/unit norm, energy ratios, rank diagnostics).

## If a gate HALTS — read this

**A halt is not a failure. It is the reportable outcome of EXP075**
(protocol §7.2). Do NOT adjust the subspace construction, energy bar, or
headroom handling and re-run under the EXP075 label — that is forbidden (it
would make the gates p-hacking-adjacent). Instead:

1. Download `exp075_results.json` + `exp075_run_log.txt` (the halt diagnostics).
2. Run `evaluate_exp075.py` — it will print the branch ruling verbatim:
...
Send back `exp075_results.json` (and the log). The ruling from
`evaluate_exp075.py` determines the next step per the pre-registered tree —
