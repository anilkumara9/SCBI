...
Usage:
    python test_evaluate_exp075.py
"""
...
from evaluate_exp075 import evaluate, normalize_exp075, report
...
def main():
    ...
    _halt_energy = normalize_exp075({
        "experiment": "EXP075", "outcome": "HALT_ENERGY",
    ...
    _halt_rank = normalize_exp075({
        "experiment": "EXP075", "outcome": "HALT_RANK",
        "protocol_scope": "IN-SCOPE (pythia-410m, layer 20)",
    ...
    # F9: malformed non-halt PAYLOAD (through normalize_exp075) -> the
    # "unclassified" fail-safe, not a KeyError crash; report() stays intact.
    _malformed = normalize_exp075({"experiment": "EXP075", "outcome": "COMPLETED",
                                   "protocol_scope": "IN-SCOPE (pythia-410m, layer 20)",
    ...
    print()
    if ok:
        print("ALL EXP075 EVALUATOR BRANCH TESTS PASSED")
    else:
