"""
EXP028c Phase B: Confirmatory Benchmark on BLOOM-560M.

Evaluates the permanently locked prospective layer l* from Phase A (Layer 13, lambda = 0.542)
against the pre-declared heuristic baseline Layer 16 (lambda = 0.667) and runner-up Layer 14 (lambda = 0.583)
on BENCH-002-NL (N=100, Seed 84).

Primary Endpoint:
- Delta M_Oracle(l*) = M_Oracle(l*) - M_Identity > 0
- Comparative Test: l* (Layer 13) vs. Layer 16 (Exact Paired McNemar Test & Paired Wilcoxon)
...
def main():
    print("=" * 100)
    print("EXP028c PHASE B: CONFIRMATORY BENCHMARK (bigscience/bloom-560m)")
    print("Evaluating Locked Prospective Stage l* vs. Pre-Declared Baseline Layer 16")
    print("=" * 100)

    lock_file = "experiments/runs/EXP028c_bloom/exp028c_prediction_lock.json"
    ...
    # 1. Evaluate Locked Layer l* (Layer 13)
    ...
    # 3. Evaluate Runner-up Stage (Layer 14)
    ...
    # 4. Pairwise Confirmatory Test: l* (Layer 13) vs. Baseline Layer 16
    ...
    out_file = "experiments/runs/EXP028c_bloom/exp028c_confirmatory_results.json"
    with open(out_file, "w") as f:
        json.dump(final_report, f, indent=2)

    print("\n" + "=" * 100)
    print("EXP028c PHASE B BENCHMARK RESULTS:")
    print("=" * 100)
    ...
    print(f"Runner-up L14:       M_Oracle = {res_runner_up['M_Oracle']:.4f} (Headroom = {res_runner_up['Oracle_Headroom']:+.4f})")
    print("-" * 100)
