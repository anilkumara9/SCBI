...
**Status:** DRAFT v2 — UNSIGNED. Not cleared for bundle build or execution.
**Supersedes:** `EXP092_IBL_PREREG_DRAFT.md` (v1), which was REJECTED at LOG-4319 (see §0).
...
| 3 | Permutation null could not calibrate the entity-set confound | **Entity-set oracle explicitly constructed and tested**: leave-one-out 1-NN by max premise-entity overlap, random tie-break, same stratified permutation scheme | Oracle acc = **0.0607** (6.07%), permutation p = **0.567** (10,000 perms) — oracle is AT CHANCE and NOT significant under the null; the null **validly calibrates** the confound |
...
- **Bench hash** (SHA-256 over canonical JSON, `sort_keys=True`): `2996ac8b27db782621a332916eb12cfcdbb429e281b8fe0ee7237e0053e3ac3e`
...
- **G0 (new-bench provenance)**: bundle asserts bench SHA-256 = `2996ac8b…3ac3e`; 60/60 unique `(domain, tuple)` keys; 20/20 targets at exactly 3 occurrences/domain; 30/30 A-first/C-first. Else RUN-INVALID.
- **G1 (Δθ=0)**: state_dict SHA-256 (sorted keys, float32 bytes — the EXP077/EXP091 `compute_state_dict_hash` procedure) must equal the LOG-331 pin `ec276abe3902fab0166ce56c00de84c2c737c80af4f9c41a9cbe94e4ec38e0ed`, checked pre- AND post-extraction. **CAVEAT (2026-09-25)**: the weights were relocated from `experiments/runs/EXP086_amplifier/weights/pythia-410m/` to `/home/hatch/workspace/.exp086_weights/pythia-410m/` (to keep the repo committable). A same-day recomputation using the bundle's exact hash procedure did **NOT** reproduce the LOG-331 pin against the relocated snapshot. The relocation is presumed content-preserving (911,373,632 bytes, 364 tensors, loads cleanly), but **the hash mismatch is unresolved**. The bundle MUST verify G1 at runtime; on mismatch it MUST RUN-INVALID, not proceed. This draft does not license execution until the Law #14 reviewer adjudicates the weights-integrity question. (Tokenizer verified loading from the new path; G1′ re-verified 240/240 against it.)
...
- **G2 (null calibration)**: the bundle runs the §8 entity-set oracle diagnostic (10,000 stratified perms) on the registered bench at build time; asserts oracle p ≥ 0.05 (oracle NOT significant). If the oracle is significant, the bench is confounded → RUN-INVALID (do not silently "fix" the bench; re-register).
...
**Oracle definition** (the exact confound the Law #14 review named): leave-one-out 1-NN where similarity(i,j) = |premise-entities(i) ∩ premise-entities(j)| (max overlap); ties broken uniformly at random (expected accuracy over tie draws); label = target entity of the nearest neighbor.

**Result on the registered EXP092-B bench** (computed 2026-09-25, deterministic seed 1022):
- Oracle accuracy = **0.0607** (6.07%) — vs 5% chance for K=20.
- Stratified permutation null (10,000 perms, same strata as §4): **p = 0.567**. NOT significant.
...
Prompt construction (identical templates to EXP091): for triple index i in planetary, A-first iff i is odd; in elemental, A-first iff i is even. For quads the same parity rule applies. A-first items use "outranks" phrasing, C-first items use "is lower than" phrasing (8 A-first per 15-block in planetary, 7 in elemental → 30/30 total).

## Appendix B. Oracle confound proof (execution record)

Computed 2026-09-25 on the registered bench (seed 1022), deterministic:

- Entity-set oracle: leave-one-out 1-NN, similarity = |premise-entities(i) ∩ premise-entities(j)|, uniform-random tie-break (expected accuracy).
- Oracle accuracy: **0.0607** (6.07%).
- Stratified permutation null (10,000 permutations, same domain × phrasing strata as §4): **p = 0.567**.
- Verdict on the diagnostic: oracle NOT significant → null calibrates the confound.
