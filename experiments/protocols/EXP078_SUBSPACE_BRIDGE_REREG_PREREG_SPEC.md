# EXP078 Protocol Specification: Subspace-Restricted Bridge (Re-registration) — Is the Subspace Right and the Direction Wrong?

**Status:** DRAFT — pending Law #14 adversarial review (becomes PRE-REGISTERED on SIGN)
**Date:** 2026-09-23
**Author role:** Preregistration Agent (re-registration of EXP075, `experiments/protocols/EXP075_SUBSPACE_BRIDGE_PREREG_SPEC.md`)
**Predecessor experiments:** EXP065/EXP066 (boundary series; the static null and the bridge rescue); EXP075 (SIGNED, bundle cleared LOG-107, aborted pre-execution — see §0)
...
`research/MATH_STANDARDS_CHARTER.md` (M1–M8)
...
## 0. Re-registration note [FACT — procedural record]

EXP075 was SIGNED and its execution bundle cleared for execution (LOG-107). On
2026-09-23, the bundle's first real startup smoke test against the actual
`EleutherAI/pythia-410m` tokenizer triggered the review-mandated F2 single-token
guard (LOG-100, finding F2; fix LOG-105): entity 'Caleb' encodes to 2 tokens
(`[330, 43705]`) under the registered tokenizer, and the guard aborted the run
before any forward pass. A full audit of all 35 support+test entities against the
real tokenizer found **11 multi-token entities**, all in the support vocabularies:

- V2_Biblical: Caleb, Miriam, Reuben
- V3_Greek: Hector, Nestor, Priam
- V4_Roman: Lucius, Titus, Silas
- V5_Modern: Sora, Leila

All 10 benchmark novel entities (planets/elements) are single-token — the 60-item
benchmark is unaffected. [FACT] The historical EXP066 410m replication script
(`experiments/scripts/run_exp066_pythia410m_replication.py`) handled these entities
via `tokenizer.encode(" " + e)[0]` — silent first-subtoken truncation, undisclosed.
The F2 review-mandated abort exists precisely to forbid reproducing that silent
truncation; the guard worked exactly as designed, catching at smoke-test time (the
LOG-108 process lesson validated) what unit tests with mock tokenizers missed.

Per Law #4 (design changes require new experiment numbers), the corrected design is
re-registered here as **EXP078** rather than amending EXP075. EXP075 remains on
record as the aborted attempt; its guard firing is itself a logged result (LOG-111).
The ONLY design delta from EXP075 is the entity substitution table in §3.1. A
disclosure correction is also carried in §8 (forward-pass budget 420 → ≈1,080 per
LOG-100 finding F3); no procedure changed.
...
[INTERPRETATION] EXP078 is a **boundary-characterization and loop-motivation experiment**,
...
- **Model / tokenizer:** `EleutherAI/pythia-410m` (identical to EXP066/EXP067/EXP075).
...
support contrast pairs, with the entity substitution table below applied. Define
...
**Entity substitution table (the sole design delta from EXP075; §0):**

| Vocab | EXP075 entity (multi-token) | EXP078 substitute | Tokenizer check (`" "+name`, pythia-410m) |
|---|---|---|---|
| V2_Biblical | Caleb | Joel | 1 token |
| V2_Biblical | Miriam | Ruth | 1 token |
| V2_Biblical | Reuben | Abel | 1 token |
| V3_Greek | Hector | Ajax | 1 token |
| V3_Greek | Nestor | Apollo | 1 token |
| V3_Greek | Priam | Atlas | 1 token |
| V4_Roman | Lucius | Julius | 1 token |
| V4_Roman | Titus | Augustus | 1 token |
| V4_Roman | Silas | Diana | 1 token |
| V5_Modern | Sora | Eli | 1 token |
| V5_Modern | Leila | Finn | 1 token |

Each substitute was verified to encode as exactly one token under the pythia-410m
tokenizer with the leading-space convention used throughout the procedure, and to
collide with no existing support, benchmark, or foil entity. Thematic grouping is
preserved (Biblical/Greek/Roman/Modern names). Unchanged entities: V1_Anglo
(Alice/Bob/Charlie/David/Emma) in full; V2 Aaron/Gideon; V3 Jason/Paris; V4
Marcus/Felix; V5 Liam/Noah/Maya.

**Provenance honesty [INTERPRETATION]:** EXP078's support set is intentionally NOT
item-identical to the historical EXP066 replication script — item-identity would
require reproducing that script's undisclosed silent first-subtoken truncation
(`encode(" " + e)[0]`), which the Law #14 F2 review forbids. EXP078 is item-identical
*in procedure* with corrected entities. The contrast-pair structure (5 vocabs × 30
pairs, templates, alternation, foil/target assignment) is otherwise unchanged from
EXP066/EXP075.
...
entities are person names while test entities are planets (Mars/Venus/Jupiter/Saturn/
Mercury) and elements (Iron/Gold/Silver/Bronze/Steel). Concretely, the construction
script maintains a path whitelist containing only the support artifact directories:
any attempt to open a path outside the whitelist (in particular any test-benchmark
path) raises a runtime assertion and aborts the run before any vector is constructed.
$S$ is therefore fixed before the first test item is seen.

**Single-token guard (F2, pre-registered, abort on violation):** after tokenizer load
and before any forward pass or support construction, every support and test entity is
checked as the exact encoded string `" " + e`; any entity encoding to other than
exactly one token triggers FATAL abort naming the entity (Law #2-adjacent: multi-token
entities are never silently truncated to the first subtoken). The EXP078 entity lists
above pass this guard against the real pythia-410m tokenizer (verified 2026-09-23);
the guard remains armed at execution as defense-in-depth.
...
| C4 | **Subspace-restricted bridge (EXP078 test)** | §3.5 — primary test condition |
...
- **Benchmark:** the identical N=60 Planetary/Elemental 2-hop/3-hop suite from EXP065/066
  (same items, same premise permutations — byte-identical; the benchmark entities are
  all single-token and untouched by the §3.1 substitution). No new benchmark
  construction (Law #9).
- **Headroom gate:** run C1 first; require baseline accuracy $\in [40\%, 70\%]$.
  ...
  A headroom halt is reportable per §7.2, not a trigger for adjustment under EXP078.
...
EXP078 has three halt gates: the rank guard (§3.1), the energy gate (§3.4), and the
headroom gate (§5). **Any halt IS the published outcome** — a halted EXP078 is a result
...
or headroom handling under the EXP078 label. Any adjusted design takes the **next free
number at pre-registration time** (EXP075→EXP078 precedent; EXP070 minor-m1 precedent);
see §9 for reservations.

## 8. Reproducibility and hygiene

- **Seeds (pre-registered):** `torch.manual_seed(20260923)`, NumPy `20260923`;
  $B_\perp$ seed 9876 (inherited). Thin-QR is deterministic (no seed needed). All
  seeds identical to EXP075 — the entity substitution changes construction *inputs*,
  not the registered randomness.
...
- **Budget (corrected disclosure, LOG-100 F3):** ≈ **1,080 forward passes** (pythia-410m):
  300 support construction + 2 $B_{\mathrm{wrong}}$ + 60 baseline + 720 conditions
  (the EXP075 spec's "420" counted conditions only and omitted support — same
  procedure, corrected accounting). ≈ **< 45 min on a Kaggle T4** (sprint estimate
  scaled); subspace construction and projections are vector arithmetic (negligible).
  Fits comfortably in free-tier quota.

## 9. What EXP078 does NOT test, and numbering hygiene
...
- **Numbering:** changed designs take the next free number at pre-registration time.
  This experiment is EXP078 (re-registration of EXP075 per Law #4; the EXP075→EXP078
  precedent now joins the EXP070 minor-m1 precedent). Reserved: EXP069 (sprint Idea 1,
  compositional factorization), EXP071 (sprint Idea 3, steerability diagnostic),
  EXP072/073/074 (sprint Ideas 4/5/6), EXP076 (sprint wildest credible: cross-model
  causal transfer). EXP075 remains the aborted predecessor — its number is retired,
  never reused.

---

**Pre-registration checklist:** ☐ §0 re-registration note recorded ☐ entity substitution
table recorded (§3.1) with real-tokenizer verification ☐ F2 single-token guard armed
against the real tokenizer ☐ $S$ construction fixed (support-only, rank guard armed)
☐ energy gate armed ($e \ge 0.10$, sensitivity bands 5%/15%) ☐ $\alpha=0.50$ fixed
☐ seeds fixed (identical to EXP075) ☐ headroom gate armed ☐ SHA-256 guard armed
☐ falsification criterion recorded ☐ full decision tree recorded (§7.1, precedence stated)
☐ LICENSES / DOES-NOT-LICENSE on every branch ☐ §1.1 boundary-science scope acknowledged
☐ numbering reservations recorded ☐ budget disclosure corrected (≈1,080, LOG-100 F3)
☐ provenance honesty recorded (not item-identical to EXP066 — §3.1).
