# EXP084 Protocol Specification (DRAFT — NOT SIGNED): R2 Newton-vs-gradient duel

> **DRAFT WATERMARK — NOT A SIGNED PROTOCOL.** This skeleton discharges the ten
> LOG-260 findings against the R2 packet. It licenses **nothing**: no runner may
> be built, no GPU may be spent, no efficacy claim may be made. The next gate is
> a **fresh Law #14 re-review of this skeleton**; only a SIGN on a final
> revision promotes it toward implementation. Signed protocols are immutable —
> this draft will be superseded by a new file on promotion, never edited into
> signed status.
...
margin proxy.
**Date:** 2026-09-24. **Status:** DRAFT for Law #14 re-review. **Cost:** $0 to
draft (CPU only, no weights touched, no signed artifacts edited).
...
| INVALID (iii) | Arm (b) defined on < 12/24 items (κ̂_dir ≥ −κ_floor — the corrected §3 defined set) | **UNDEFINED-LANDSCAPE.** The duel cannot run; not a family kill. |
...
| F7 "measured κ" misnomer | κ̂_dir = directional curvature; too-flat rule pre-registered; too-noisy UNDETERMINED (single δ, replication declined, limitation recorded). **R1/R3 re-derivation (LOG-262):** definedness guard corrected — defined iff κ̂_dir < −κ_floor (concave/maximization regime); attribution splits convex-uphill (κ̂_dir ≥ +κ_floor) vs too-flat (\|κ̂_dir\| ≤ κ_floor) exclusions | §3, §7 |
...
DRAFT → Law #14 re-review (SIGN / SIGN-WITH-FIXES / REJECT) → final signed
skeleton (new file; this draft never edited into signed status) →
implementation bundle → bundle review → CEO GPU clearance → queue. $0 spent
to this point; CPU only; no weights touched; no signed artifacts edited.
