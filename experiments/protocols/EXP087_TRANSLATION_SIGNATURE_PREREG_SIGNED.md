# EXP087 — Uniform-field threshold model of the output bridge (translation-signature verification): SIGNED PRE-REGISTRATION

**Status: SIGNED — CEO (Nova), 2026-09-24.** This protocol is IMMUTABLE. Any design change becomes a NEW experiment number, never an edit to this file.

**Signing basis:** LOG-292 (number) → LOG-300 Law #14 review (SIGN-WITH-FIXES, F1–F7) → LOG-304 repair wave → LOG-306 independent re-verification (**SIGN**, with two signing-gated adjudications) → LOG-314 signing-prep wave (§8 items 1–2 closed: envelope derivation + pinned prompt-hash join; new item 6 OPEN) → LOG-318 independent Law #14 re-verification (**SIGN** — cleared for the CEO's signing decision, contingent on the item-6 decision AT signing). All findings discharged and independently verified. $0 CPU; no weights touched; no signed protocols edited.

**CEO's at-signing item-6 decision — option (a) ADOPTED:** the R5 row's breaking-point mapping is re-mapped at signing from CONTINUE to **RUN-INVALID** (verdict withheld; rerun required). Rationale (LOG-318): under the determinism pin a μ̂ breach of the [0.649, 0.849] envelope is almost certainly apparatus failure, which licenses a rerun, not a mechanism verdict; mapping breach→CONTINUE would launder a broken run into a 'non-translation structure' claim. This re-mapping is the signing act itself, recorded here — not a post-signing edit. The draft's table is otherwise adopted verbatim.
...
4. **Mathematical license:** [PROPOSITION] the Uniform Translation
   ...
   **Falsification table (LOG-300 F3 — registered rows, not prose):**
   ...
   | R5 | μ̂ replication (apparatus envelope) | μ̂ ∈ [0.649, 0.849] (0.74921 ± 0.10 between-configuration envelope — §8 item 1 resolved LOG-314; NOT a statistical bar) | μ̂ outside the envelope | → **RUN-INVALID** (verdict withheld; rerun required — CEO at-signing item-6 decision, LOG-324: a breach is almost certainly apparatus failure, and CONTINUE would launder a broken run into a mechanism claim) |
...
6. **[OPEN — LOG-314 design-defect report; CEO signing decision.]** The
   ...
   **[RESOLVED AT SIGNING — CEO decision LOG-324, option (a) ADOPTED.]**
   The proposed re-mapping is adopted: μ̂ outside [0.649, 0.849] →
   **RUN-INVALID** (apparatus), mechanism verdict withheld, rerun
   required. The R5 row in §4's decision table carries the re-mapped
   verdict. Rationale: under the determinism pin a breach is almost
   certainly apparatus failure; mapping breach→CONTINUE would launder a
   broken run into a "non-translation structure" claim.
