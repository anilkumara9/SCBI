# EXP089 — Zero-Shot Contrastive-Style Scoring over Frozen Representations: SIGNED PRE-REGISTRATION
...
**Signing basis:** LOG-334 (CLM-8B frontier-adaptation analysis; verdict ADAPT, brutal caveats on record) → LOG-335 (EXP089 draft, free-lunch falsifier) → LOG-336 independent Law #14 review (**SIGN-WITH-FIXES**, F1–F6; reviewer reports to the founder directly) → LOG-337 repair wave (all six fixes applied; CEO-verified present by marker grep). $0 CPU; no weights touched; no signed protocols edited.

**CEO's signing note:** signed EXPECTING a kill-or-signal — the CLM-8B free-lunch version dies if zero-shot cosine scoring over frozen states cannot clear 38/60, and either outcome is licensed science. No trained heads, no weight updates: Δθ=0 is a scope fence, not a suggestion — any CONTINUE toward trainable apparatus requires a NEW experiment number + the founder's explicit license.

**Licensed:** execution-bundle build (CPU) may proceed. Execution is CPU-only — no GPU clearance required or granted. Queue: none; this is the program's fastest live experiment (~1–2h CPU).

---
# EXP089 — Zero-Shot Contrastive-Style Scoring over Frozen Representations
## (CLM-8B adaptation: the free-lunch falsifier)

**Status line superseded by the SIGNED header above (LOG-338, 2026-09-25).**
**Experiment number:** EXP089 (verified free 2026-09-25; no EXP089 file existed in
...
> **DRAFT WATERMARK.** This document is a draft pre-registration. It is unsigned,
> unreviewed, and licenses NOTHING. Execution requires the full launch chain:
> signed pre-registration → independent Law #14 review → startup smoke test →
> CEO GPU clearance (not applicable; CPU-only) → execution. The runner must
> REFUSE any execution flag while this file is unsigned.
...
## 4. Artifact correction (memo §5 assumption REVISED)

The adaptation memo's §5 sketch assumed `exp077_vectors.pt` contains per-item
state/option embeddings. **Artifact inspection (2026-09-25, read-only) shows
this is FALSE:** the file contains 18 tensors — sixteen 1024-dim float32
direction vectors (keys `v_hat`, `v_hat_c`, `v_hat_c_ks`, `v_hats`,
`B_wrong`, `B_perp_basis`, `mu`, `q_list`, `r_vec`, `u_list`) and two
32,768-byte basis blocks — i.e., the cone experiment's *direction vectors*,
not per-item activations.

- SHA-256 (recorded read-only, file NOT modified). **Full paths [Law #14 F5]:**
  all four artifacts live in `experiments/runs/EXP077_cone_vs_line/`; the
  benchmark builder code lives in `experiments/runs/exp077/`:
...
- **Revised executor contract:** per-item embeddings are re-extracted from the
  frozen Pythia-410m (CPU, read-only) using EXP077's registered benchmark
  construction (§6). The LOG-331 weight snapshot
  (`experiments/runs/EXP086_amplifier/weights/pythia-410m/`) is reused; its
  state_dict SHA-256 (`ec276abe3902fab0166ce56c00de84c2c737c80af4f9c41e1b53959d61be7d2d8bb90f7`)
...
**Benchmark (Law #9: no new benchmark construction).** Port the EXP077
benchmark builder VERBATIM from `experiments/runs/exp077/run_exp077.py`
(N=60 Planetary/Elemental 2-hop/3-hop, same vocab lists, same index arrays,
same seeds). Assert `len(bench)==60` and that the (ent, typ) sequence matches
`exp077_instance_records.json` byte-for-value. Mismatch → RUN-INVALID.
...
- **Guards (all FATAL/INVALID on failure):**
  ...
  - G3: phrasing-balance assert — exactly 30 "A-first" items
    (`{A} outranks {B}...`, i<8 planet / i<7 element) and 30 "C-first" items
    (`{C} is lower than {B}...`); the builder MUST assert this count.
...
Exact: P(X ≥ 38 | n=60, p=0.5) = **0.0260** (computed exactly; the memo's
≈0.037 was a normal approximation — this draft registers the exact value).
...
**RUN-INVALID (withheld, not a verdict):** weight-hash mismatch (G2); any
multi-token A/C (G1); benchmark regeneration mismatch vs
`exp077_instance_records.json`; phrasing-balance assert failure (G3); N ≠ 60
for any reason; **instrument deafness (G4):** `max − min < 1e-4` over the 120
cosine values → RUN-INVALID, never KILL (F1).

**Refusal:** the execution bundle (when built) MUST refuse any run while this
protocol is unsigned, and MUST refuse any flag requesting training, GPU
execution, or weight mutation. Draft status confers zero license.
...
EXP077 benchmark (Law #9), the on-disk LOG-331 weight snapshot, and one
CPU-hour. If frozen geometry carries no signal, the adaptation dies here —
before a single dollar or GPU-hour is contemplated for head training.
...
*Draft prepared by the pre-registration agent, 2026-09-25. All artifact
hashes and the binomial bar were computed from actual inspection in this
session; the P(X≥38)=0.0260 value supersedes the memo's ≈0.037 approximation.*
