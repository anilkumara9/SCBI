# K2 — Routing-vs-Bypass Discriminator: REV1 Pre-registration

**LOG-223 · Track-8 pre-registration drafter · 2026-09-23 · REV1**
**Status:** FROZEN (revision of frozen LOG-223 plan; the frozen original is untouched).
**Revised under:** Law #14 SIGN-WITH-FIXES, LOG-224 (2026-09-23).
**Targeted re-check:** LOG-224b (pre-assigned) — changed sections only.
**DO NOT EXECUTE before Law #14 SIGN (targeted re-check LOG-224b, pre-assigned) and CEO GPU clearance.**
...
1. **Bench pin (CEO decision).** K2-P's 410m pin is pinned to the **EXP077 smoke bench**, not the official bench the frozen plan cited. Exact pinning:
   ...
   - Per-item bridge vectors: `experiments/runs/EXP077_cone_vs_line/exp077_vectors.pt` (per-item `v_hat`; verified by the LOG-224 reviewer).
...
5. Artifact existence checks (no execution): `experiments/runs/EXP077_cone_vs_line/exp077_instance_records.json` (60 items, schema `item`/`ent`/`typ`/`correct`/`rescue_indicators`) and `experiments/runs/EXP077_cone_vs_line/exp077_vectors.pt` (per-item `v_hat`; torch-zip structure intact); rescue anchor from `exp077_results.json` → `bridge_gate`: **b=14, c=0, Δm=+0.233333, p=0.0001220703125** [FACT].
...
|---|---|---|---|---|---|---|
| **K2-P (primary)** | `EleutherAI/pythia-410m` | L20 | 0.5 | per-item archived `v_hat` = normalize(E[target]−E[foil]) (`experiments/runs/EXP077_cone_vs_line/exp077_vectors.pt`; LOG-197 E1+E2 identity chain) | EXP077 **smoke** 60 items (`experiments/runs/EXP077_cone_vs_line/exp077_instance_records.json`) | **+23.33pp, b=14, c=0, p=0.0001220703125** (smoke `bridge_gate`; baseline acc 0.60) |
...
- **Routing Supported / final-position-local Not supported:** L(Δ̂M_b − Δ̂M_a) > 0.05 AND b_b ≥ 6 (L1 MDE at c=0) AND U(Δ̂M_a) < 0.05 (the final-position channel demonstrably flat).
...
- **Source:** in-house proof (elementary induction on the causal mask; M4 space/shape checks trivial — positions and layers are the objects). Proof document to be attached at the LOG-224b re-check; the K2 run is graded as the license's empirical exercise, not its proof.
- **Epistemic label:** [PROPOSITION] (→ [THEOREM] after Law #14 adversarial review of the proof).
- **Companion identity [PROPOSITION]:** b_archived = α·normalize(E[t]−E[f]) per item (LOG-197 E1+E2 chain); discharged per guard G1 against `exp077_vectors.pt`.
...
- **Promotion path:** the lemma to [THEOREM] via Law #14 adversarial review of the standalone proof (LOG-224b scope); the causal upshot of any K2 verdict stays at mechanism-adjudication (L1), never a capability claim.
...
- K2-P executes only after: (i) Law #14 SIGN (LOG-224b targeted re-check), (ii) CEO GPU clearance. ~~(iii)~~ — the S3-8 pre-execution gate is **discharged**: S3-8 (LOG-225) Supported → **PROCEED** (recorded, not re-litigated).
...
## 10. Execution release checklist (for the LOG-224b reviewer)
...
- [ ] Steelman graded — six entries at full strength (a weak steelman fails the review same as a weak license).
