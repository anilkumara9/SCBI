# K3 — Law-#7-Compliant Bridge Construction Audit + Rescue Screen: FROZEN PLAN (R1)

**LOG-239 · Plan K3-Phase0-R1 · Track-5 (AGI architecture) / Track-6 (systems) specialist · 2026-09-23**

> **FROZEN — DO NOT EXECUTE before Law #14 SIGN (LOG-240).**
> Execution is released only by an independent Law #14 review SIGN under workstream
> LOG-240 (reviewer assigned by the Research Lead; must not be the plan author).
> No GPU, no weights loaded, no forward passes have been run in the preparation of
> this plan.

**Law #4 chain (re-registration reason):** This plan RE-REGISTERS, and does not edit,
the frozen plan
`research/analysis_plans/K3_COMPLIANT_BRIDGE_PLAN_LOG234_2026-09-23.md`
(Law #14 SIGNED via LOG-235/236/237). That plan's Phase-0 execution (LOG-238)
HALTED at §10 step 2 on a FATAL G4 caused by **plan-premise defect D1** — G4's
label/premise cross-check was pinned to the in-repo **pre-MAJOR-3-repair smoke
archive** (`exp077_instance_records.json`: entity-grouped 6/entity, (b,c)=(14,0)),
while the plan's bench is the **post-repair bench** (planet targets 21/7/2/0/0;
Saturn/Mercury never targets): 12/60 matches, 48 mismatches
(`research/analysis_plans/K3_PHASE0_HALT_REPORT_LOG238_2026-09-23.md`, §3).
The single change from LOG-234 to R1 is: **G4's cross-check is repointed from the
pre-repair smoke archive to the signed pre-audit JSON's per-item records**
(`experiments/protocols/C-A_PREAUDIT_2026-09-23.json`,
`similarity_audit.per_item`: 60 records with `(id, t, f)` fields) — everything
else carries over verbatim, including every endpoint, guard, table, pin, and
tolerance. Per the LOG-238 recommendation, the re-registration does NOT "repair"
the smoke archive (Law #8 — it faithfully records its run's bench) and does NOT
weaken G4 (the cross-check remains FATAL and load-bearing for A1).
...
- **Cross-check (guard §8 G4):** the rebuilt (B_i, C_i, target_i, foil_i) strings
  must be byte-consistent with the **signed pre-audit per-item records**:
  `experiments/protocols/C-A_PREAUDIT_2026-09-23.json`,
  `similarity_audit.per_item` — a list of 60 records, each with fields
  `(id, t, f)` (example record: `{"id": "exp077_planet_2hop_0", "t": "Mars",
  "f": "Jupiter", ...}`). The executor rebuilds `(id, t, f)` per item and asserts
  **60/60 byte-agreement**; any item mismatch = FATAL. Expected agreement: 60/60.
  The LOG-234 plan pinned this cross-check to the smoke archive
  `exp077_instance_records.json` — that pin is **superseded** by R1 (see the
  D1-lesson note below); the smoke archive is not a cross-check source.
- **D1-lesson note (binding — added in R1):** [FACT, per LOG-238] The in-repo
  `exp077_instance_records.json` archive is **pre-MAJOR-3-repair smoke**
  (entity-grouped 6/entity per target; its run's bench is the Anglo-subjects
  "visited/forged" bench identified in LOG-3378 MAJOR-3, not the pinned post-repair
  bench from LOG-3381). It must **never** be used as a cross-check source for the
  post-repair bench: the post-repair bench yields planet targets
  {Mars: 21, Venus: 7, Jupiter: 2, Saturn: 0, Mercury: 0}, structurally
  unreconcileable with the archive's entity-grouped layout. LOG-197's "byte-identical
  60/60" sentence covers the runners' *current* (post-repair) item definitions, not
  the smoke archive — the LOG-234 plan mis-scoped it. Same defect class as
  LOG-197 F1 (smoke/official conflation) and EXP082's D1 (hash-formulation pin);
  Law #8 keeps the smoke archive untouched as a faithful record of its run's bench.
  Any future cross-check source must be (i) signed and (ii) verified to record the
  post-repair bench — the signed pre-audit JSON satisfies both; the smoke archive
  satisfies neither.
...
**E-K3-5 — machine-readable twin.** `K3_CONSTRUCTION_AUDIT_RESULTS_LOG239_2026-09-23.json`:
unrounded g values, per-pair norms, per-item cosine arrays, token-id audit sets,
hashes, env manifest. Plus the executor script
`K3_construction_audit_execute_LOG239_2026-09-23.py` (filed with the report;
...
`K3_rescue_execute_LOG239_2026-09-23.py` from this frozen spec at release; no script
...
- **A1 — item/premise labels:** verbatim ports of the runner's bench-construction
  ...
  §8 G4 cross-check vs the **signed pre-audit per-item records** (`similarity_audit.
  per_item`, 60 `(id,t,f)` records); expected 60/60 byte-agreement; any item mismatch
  = FATAL.
...
- **G4.** Label/premise cross-check (§2): the executor's rebuilt per-item
  `(id, t, f)` records are byte-compared (index-aligned, exact strings) against the
  **signed pre-audit JSON** `experiments/protocols/C-A_PREAUDIT_2026-09-23.json`
  `similarity_audit.per_item` (60 `(id, t, f)` records). **Any item mismatch =
  FATAL** (expected agreement: 60/60). **The executor RECOMPUTES this comparison
  itself from the source JSON file** — it does not inherit, quote, or otherwise
  rely on the LOG-238 run's 60/60 byte-agreement claim (Law #3:
  recompute-don't-trust; the LOG-238 60/60 agreement is a claim to verify, not a
  fact to inherit). Cross-checking against the pre-repair smoke archive
  (`exp077_instance_records.json`) is prohibited (the §2 D1-lesson note, binding).
...
- **Phase-0 PROCEED (≥1 Supported):** the supported candidate(s) advance to the §8 G9
  release chain. Nothing else changes; no verdict on H_compliant yet. **R1-specific
  consequence (pre-registered):** if R1's repointed G4 passes (60/60, recomputed by
  the executor per Law #3), Phase-0 proceeds under this re-registration and the
  LOG-238 halt **converts to a superseded halt** — the LOG-238 halt report is
  preserved in place, never deleted or edited (Law #8); the record shows halted-
  then-superseded, not halted-and-erased.
...
2. Port the bench-construction loops verbatim (`run_exp077.py` ll. 586–636; record
   exact quad-loop sub-ranges); rebuild (B_i, C_i, target_i, foil_i) per item;
   **G4 cross-check vs the signed pre-audit JSON
   `experiments/protocols/C-A_PREAUDIT_2026-09-23.json`
   `similarity_audit.per_item` — index-aligned, byte-exact per-item (id, t, f);
   expected 60/60; any mismatch = FATAL.** The executor recomputes the full 60/60
   comparison itself from the JSON file (Law #3); it inherits nothing from LOG-238's
   claim. It must not cross-check against `exp077_instance_records.json` (the §2
   D1-lesson note, binding). G6 single-token assert on every entity.
...
6. Write `K3_RESCUE_RESULTS_LOG239_2026-09-23.json` + report; evidentiary level L1
...
*Author: Track-5/Track-6 specialist (LOG-239) · 2026-09-23 · FROZEN pending
independent Law #14 review SIGN under LOG-240 (reviewer assigned by the Research
Lead; must not be the plan author). Plan number: **K3-Phase0-R1**; re-registers
`research/analysis_plans/K3_COMPLIANT_BRIDGE_PLAN_LOG234_2026-09-23.md` (Law #4
chain: LOG-234 SIGNED via LOG-235/236/237 → LOG-238 halt on FATAL G4, plan-premise
defect D1 → R1 repoints G4 to the signed pre-audit JSON per-item records, all else
verbatim). Experiment number: none consumed (K3 is the battery slot, not
