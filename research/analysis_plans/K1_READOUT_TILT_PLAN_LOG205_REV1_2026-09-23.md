# K1 — Readout-Tilt Falsification: FROZEN PLAN (REV1)

**LOG-205 · Track-8 (Experimental Statistician) · 2026-09-23 · REV1 (LOG-213)**
**Status:** FROZEN — pending targeted re-verification of REV1 diffs (LOG-213). **DO NOT EXECUTE before SIGN.**
**Cost:** $0 CPU. Zero forward passes. Read-only weight-tensor reads only.
**Scope banner:** this plan computes K1(a), K1(b), K1(c2-integrity), K1(D1) and
verdicts them. Endpoint (c) as a discriminating endpoint is STRUCK per CEO ruling
LOG-212 option (iii); K1 rests on (a)+(b)+(c2)+D1. The foil-suppression-tilt
question is fresh-pre-registered as EXP082 — it inherits nothing from K1's battery.
This plan does NOT run K2/K3, does NOT touch EXP070 vectors (none verified —
excluded), does NOT modify any signed artifact, and does NOT authorize GPU.
**REV1 change record (vs LOG-205 as reviewed in LOG-206):** (1) §5.2 (c1) struck
per LOG-212; all (c1) kill references removed. (2) F-V1: "absent exact ties"
qualifier in §5.1. (3) F-T1: §7 row 5 reworded for literal M5.2 exhaustiveness.
(4) F-G1: A7 added (item-independence). (5) F-G2: A8 added (non-degenerate rows).
(6) F-G3: A5 line citations + tie qualifier. (7) F-S1: EXP070 labeled-secondary
permission deleted. (8) Standing correction on the adopted (b) reading recorded
(LOG-212). Nothing else changed.
...
1. **Q: What precise question does this answer?** Is the bridge's rescue power a
   ...
   pre-registered f ≥ r/2 strengthens tilt — kept verbatim per Law #4; see the
   standing correction in §4); (c2) swapped-scoring identity (§5.3 — the naive
   re-scoring is arithmetically vacuous; computed as an archive-integrity check, not
   evidence); D1 logit-shift ratio diagnostic (§5.4). **(c) as a discriminating
   endpoint is STRUCK (CEO ruling LOG-212, option iii)** — the adopted (c) is
   hypothesis-independent vacuous (LOG-206 §1(i), settled fact), and the (c1) repair
   is a design change past the mentor-adopted battery (LOG-206 §1(ii)).
...
EXP070: **excluded from all endpoints** (no verified `exp070_vectors`
record anywhere in the repo — LOG-197 E3). Its cells are Underdetermined, never
filled by assumption. The log-cited (b,c)=(10,0) is not used anywhere in this plan
(F-S1 — the labeled-secondary permission is deleted, not exercised).
...
**Kill criterion:** (a)-KILL fires → the bridge is readout bias; the "output-side
mechanism" reading is withdrawn (adopted §H6's first disjunct, implemented
faithfully — (a) alone is the effective adopted kill, LOG-206 §1(ii)(c)).
...
verbatim (no silent hypothesis shift) and rests the kill criterion on (a) alone
((c1) struck, LOG-212);
...
**STANDING CORRECTION (CEO ruling LOG-212; LOG-206 §2):** the adopted (b) reading
(f ≥ r/2 strengthens tilt) is BACKWARDS vs the program's own L1 license — L1 proves
a pure readout tilt strictly increases the t−f margin, hence predicts f = 0 exactly;
the archived f = 0, r > 0 pattern is tilt-*confirming* under the license and
tilt-*weakening* under the adopted mapping, a direct contradiction, and the license
is the proven statement. For THIS execution the adopted mapping is kept verbatim as
a diagnostic per Law #4 (the verbatim-keep is the correct no-silent-shift posture;
LOG-206 §2 ruling) and (b) does not modulate the (a)-kill. **No future tilt verdict
may lean on the adopted (b) reading until it is re-registered** (recommended
alignment: f ≥ r/2 contradicts a *clean* tilt; f = 0 is the tilt-predicted pattern).
...
mechanism hypothesis (absent exact ties under strict-`>` scoring — a tie would break
the identity; G4's FATAL-on-deviation covers the operational side). It cannot discriminate them and "rescue-persistence"
...
### 5.2 Endpoint (c1) — STRUCK as discriminating (CEO ruling LOG-212, option iii)

The swapped-construction geometric persistence endpoint (measurand
p_i = cos(−b̂_i, ŵ_{f_i}) ≥ 0.9, lower 95% Clopper–Pearson CI of p̂_c > 0.5) is a
DESIGN CHANGE past the mentor-adopted battery, not a faithful operationalization of
adopted (c) (LOG-206 §1(ii): measurand + decision rule both changed; the "binding,
from §H6" attribution was false; a (c1)-alone firing shows foil-suppression tilt,
not persistence of the (a) property; it expands the adopted kill surface). It is NOT
part of K1. The foil-suppression-tilt question it raised is fresh-pre-registered as
EXP082 with its own rationale, Law #15 answers, mathematical license, and Law #14
review — it inherits nothing from K1's battery.
...
**License grade: IN-HOUSE-PROOF** for the geometric endpoint (a) — exact
algebra on pinned weights; the computation *is* the proof; promotion to
PROVEN-LEMMA via LOG-213 targeted re-verification SIGN. **CONJECTURE-UNDER-TEST** for the causal
...
### L3. Breaking point
- **Falsifying observation (tilt):** (a)-KILL fires (§3).
...
- **Exonerating observation:** (a)-RULED-OUT on all primary runs.
...
- **Outcome partition:** §7's verdict table maps every cell of
  {(a)-fire/out/neutral} × {(b) reading} × {guards pass/
...
- **A2 — mid-network propagation (UNDISCHARGED):** the causal upshot assumes the
  ...
  license at CONJECTURE-UNDER-TEST. The (a) geometric endpoint does NOT need A2.
...
- **A5 — binary scoring (F-G3):** verified by code read (EXP077 ll. 669–682;
  EXP065 `mod_correct.append(bool(t_l_m > f_l_m))`; EXP066
  `is_mod_corr = bool(t_l_m > f_l_m)` — reviewer-verified LOG-206 §1(i); executor
  cites lines); grounds the §5.1 vacuity lemma (absent exact ties — F-V1).
- **A7 — item-independence for the Clopper–Pearson CI (F-G1):** the 60 per-item
  trials are treated as independent Bernoulli trials. Any within-run item dependence
  (shared prompt templates) is not modeled — the CI is nominal; disclosed, not hidden.
- **A8 — non-degenerate difference rows (F-G2):** the executor asserts and reports
  min_i‖W_U[t_i] − W_U[f_i]‖₂ across all items × runs. Distinct tokens can carry
  near-identical rows, which would make b̂_i noise-dominated. The minimum is reported
  as a diagnostic [OBSERVATION]; it does not void any verdict absent a guard failure.
...
4. Read archived (b, c); recompute from per-item records where available; apply §4
   reading (diagnostic).
5. Compute (c2): assert the §5.1 identity (G4). ((c1) struck — not computed.)
6. D1: archived sign signature (already in hand) + ratio diagnostic vs L1 prediction.
7. Machine-readable twin JSON (unrounded values, per-item c_i arrays, hashes,
...
No forward passes; no K2/K3 verdicts; no EXP070 endpoints (Underdetermined);
no GPU; no signed-artifact edits; no (c1) — endpoint (c) struck as discriminating
(LOG-212); no bridge reconstruction under Law #7 compliance
...
- **Challenge (answered — LOG-206 §2, binding per LOG-212):** the adopted (b)
  reading (f ≥ r/2 strengthens *tilt*) diagnoses broad disruption, while K1's
  Law-#15 Q1 asks about the *narrow* geometric tilt — for which the L1 license
  predicts f = 0. The plan kept the adopted mapping verbatim (Law #4) and rested
  the kill on (a) alone; LOG-206 ruled YES, the (b) reading must be re-registered
  before bearing weight in any future tilt verdict. The tension was kept on the
...
*Author: Track-8 (LOG-205) · REV1 mechanical fixes applied by the Research Lead
(LOG-213) per LOG-206 + CEO ruling LOG-212 · 2026-09-23 · FROZEN pending targeted
re-verification of REV1 diffs. Execution only after SIGN. On SIGN, the executor
works §10 verbatim; any deviation is logged per the deviation protocol (F3-class
table-precision corrections allowed with disclosure; endpoint/environment/margin
changes are never F3).*

**REV1 note on the §12 challenge:** answered by LOG-206 §2 — YES, the adopted (b)
reading must be re-registered before bearing weight in any future tilt verdict.
The standing correction is binding per LOG-212 (recorded in §4).
