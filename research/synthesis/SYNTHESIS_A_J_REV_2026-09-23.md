# A–J Synthesis — Revised (LOG-187)

**Commission:** LOG-140 (CEO) · **Assembly:** LOG-158 (synthesis assembler) · **Targeted revision:** LOG-187 (revision drafter, mentor-ordered)
**Date:** 2026-09-23 · **Revises:** `research/synthesis/SYNTHESIS_A_J_2026-09-23.md` (LOG-158 — immutable, untouched by this revision)
...
**Inputs:** LOG-158 original (source text) · `research/literature/mentor_citations_verification_2026-09-23.md` (LOG-185 — authoritative for every citation; nothing about the seven papers comes from the mentor review's wording) · LOG-186 EXP023 lineage ruling (authoritative for §A10 — applied as issued; the ruling's record is the research_log.md entry); the audit file was not present in the workspace at drafting time, see R(h)) · mentor review findings #1–#11
...
**Status:** revised draft for Law #14 review before program adoption; G/H pre-registration triggers on this revision landing (spec §4).
...
survive the Level-2 multiplicity budget (two-sided p=0.03125 > α₂=0.00625) —
...
verified by Cluster B via public web search; venue and 80.4% figure confirmed
per LOG-185]: test-time gradient descent on token *logits* (Differentiable
...
arXiv:2604.19018)** [FACT]/[OBSERVATION, verified per LOG-185 — ICML 2026
proceedings listing (entry 3288) confirmed]: layer-wise Jacobians linearize
...
**DEER (Yang et al., 2025; arXiv:2504.15895)** [FACT]/[OBSERVATION, verified in LOG-158 Cluster B audit — not re-verified under LOG-185]:
...
- **Family C — inference-time search + self-evaluation:** self-consistency,
  ...
  **NoisyCoconut** (Jerge & Evans, arXiv:2605.08221 — arXiv preprint
  **under review at TMLR; not a published TMLR paper**): training-free
  inference-time noise injection into latent trajectories; K diverse latent
  paths from a common initial hidden state; pairwise trajectory-diversity
  measurement; aggregation by majority or confidence-weighted voting; no
  retraining, no parameter modification. Occupies "latent branching +
  aggregation/consensus" generally.
...
3. **Matched-compute superiority over Family C/D/E/F baselines** — beats
   ...
   **plus candidate-specific forced comparators at matched compute:**
   LTPO-style latent optimization (H1-family: C1, DPRS), Meta-Reasoner-style
   strategy routing (H4: ASR), LatentMAS-style shared-latent-memory
   collaboration (H5a: LCMIC), NoisyCoconut-style branching/consensus
   (H5b: CLB).
...
> does not merely demonstrate that one of the eight mechanisms works. It must
...
is the feedback-control formulation with an error bound — and LTPO-style
latent optimization is a forced comparator for H1-family candidates at
matched compute (§G3 M15). Honest tier: N1 until the bound holds
...
> of the *same final correction* (no feedback) does not. No competitor
> predicts bound-holding feedback beating its own open-loop ablation —
> C2/C3/C6 have no feedback; C4 routes but does not control; C5 debates but
> does not track.
>
...
> LTPO-style latent optimization at matched compute on the relational task
...
LTPO-style latent optimization is the forced H1-family comparator (§G3 M15).
...
> (corrupting a banked slot flips the decision). No competitor has cross-pass
> accumulating state — C1/C4/C5/C6 recompute every pass; C3's "memory" is the
...
> pre-registered) AND the test-time bandit policy beats the fixed
> entropy-threshold policy AND the bandit policy beats or structurally
> differs from Meta-Reasoner-style routing at matched compute. No competitor
...
debate/MoA and LatentMAS-style shared-memory collaboration at matched
...
> a LatentMAS-style shared-latent-memory ablation (same roles/rounds/budget;
> lower 95% CI > δ_min) AND text-only ≈ debate/MoA forced baseline (the roles
> add nothing by themselves) AND the critic's latent doubt scalar predicts
> proposer errors above chance (the channel carries signal, not noise). If
> the LatentMAS-style ablation matches, LCMIC reduces to prior art — culled.
>
...
arXiv:2605.08221 — arXiv preprint under review at TMLR) occupies
frozen-model latent branching with aggregation: noise-injected latent
trajectories, measured path diversity, majority/confidence-weighted
consensus. CLB's surviving hypothesis is narrowed: **intra-forward/
...
> operator (lower 95% CI > δ_min) AND CLB at ~2F beats Best-of-4 at 4F (more
> capability per FLOP) AND beats NoisyCoconut-style branching/consensus at
> matched compute AND the merge-vs-select ablation favors the merge operator
> (merge ≠ argmax/selection — the decisive test). No competitor branches
> inside the pass and merges non-selectively — C4 routes between whole
> strategies, C5 parallelizes instances, C2/C3 serialize passes.
>
...
> vanishes against the NoisyCoconut-style baseline → the operator adds
...
| E7 DPRS | H1 | dynamic readout-space search (narrowed; LTPO-style latent optimization forced comparator) | pilot ~960F total | both answered |
...
| C4 ASR | H4 | test-time-learned strategy routing (Meta-Reasoner-style routing forced comparator; policy scope [INCOMPLETE]) | ~3.2F | both answered |
| C5 LCMIC | H5a | formulation-level latent-channel cooperation (same backbone + residual exchange + verifier/arbiter split); LatentMAS-style collaboration forced comparator | 6F | both answered |
| C6 CLB | H5b | intra-pass latent branching + non-selection merge operator; NoisyCoconut-style branching/consensus forced baseline | ~2F, K·A | both answered |
...
## F3 — The latent-channel ablation (C5 vs debate/MoA vs LatentMAS-style)

**Discriminates:** C5's channel claim from the forced text-debate baseline
and from LatentMAS-style shared-memory collaboration; secondarily tests
whether *any* multi-instance candidate needs its exotic channel.
**Design** [HYPOTHESIS]/[HYPOTHESIS]: four arms at matched token budget and
matched FLOPs on the NTDP task (§G): (a) C5 full (latent exchange),
(b) C5-text (identical roles/rounds, text-only communication), (c)
debate/MoA forced baseline, (d) LatentMAS-style shared-latent-memory
collaboration (same roles/rounds/budget). Pre-registered predictions:
(a)>(b) with lower 95% CI > δ_min, AND (b)≈(c) demonstrated by equivalence
(95% CI for (b−c) within ±δ_min), AND (a)>(d) with lower 95% CI > δ_min ⇒
the latent channel — in LCMIC's formulation — is the mechanism; (a)≈(b) with
the CI for (a−b) ruling out δ_min ⇒ C5 reduces to debate (culled, N1);
(a)≈(d) with the CI ruling out δ_min ⇒ C5 reduces to prior art (culled,
N1); equivalence not demonstrable (CI too wide) ⇒ Inconclusive (held, not
culled). Secondary: critic's latent doubt scalar predicts proposer errors
above chance (AUC>0.55) — the channel carries signal.
**Why it discriminates** [INFERENCE]/[INTERPRETATION]: the cheapest
family-killing experiment in the set (~6F/item × 4 arms, one task family);
it adjudicates the entire "cooperation" direction: if latent bandwidth adds
nothing over text or over LatentMAS-style shared memory, the H5-cooperation
branch of the design space is closed without further spend.
...
practical margin:** δ_min = 0.05 (5pp) — below the smallest bridge rescue
ever observed in the corpus (+10pp, EXP077 official) and above the noise
floor; with N=80 the two-sided exact McNemar MDE is ≈7.5pp at c=0, so
δ_min lies just below typical detectability — the Inconclusive cell absorbs
that gap instead of converting underpower into a kill.
...
- **Level 2 — Candidate gate (Branch-S licensing, per candidate).**
  ...
  CI rule above; CIs at Level 2 are 1−α₂ = 99.375% exact two-sided
...
α₂=0.00625 (two-sided testing), the MDE computed under the pilot-estimated
...
shuffled-label) · **M15 candidate-specific forced comparator**
(per-candidate: LTPO-style latent optimization for H1-family C1/DPRS;
Meta-Reasoner-style strategy routing for C4/ASR; LatentMAS-style
shared-latent-memory collaboration for C5/LCMIC — run as §F3's fourth arm;
NoisyCoconut-style branching/consensus for C6/CLB), at matched (a) online
inference FLOPs. Primary endpoint: ΔM decision changes, exact McNemar with
...
- **Branch S — KILLER SUCCESS.** ALL hold: (i) ΔM_C\* > 0 vs M1, with lower
  99.375% CI (1−α₂) > δ_min = 0.05; (ii) C\* beats EACH of M2–M8 at matched FLOPs, each
  with lower 99.375% CI (1−α₂) > δ_min = 0.05; (iii) M9 does not beat C\* beyond the
  ...
  fingerprint check passes; (vi) per-sample harm ledger: anti-steerable
  fraction < 15% (else Branch F4). → **Verdict: Supported** (L2 licensed).
...
*End of revised synthesis. LOG-187 revision dispatch discharged. No GPU used;
no signed protocols, primary artifacts, or the mentor review file modified.
Awaits Law #14 review before program adoption; G/H pre-registration triggers
on landing.*
